...
//        template.convertAndSend("/topic/chat/room/" + chatMessage.getRoomId(), chatMessage);
        kafkaTemplate.send("/topic/chat/room/" + chatMessage.getRoomId(), chatMessage);
    }
...
//                        template.convertAndSend("/queue/user/" + group.getUser().getId(), chatMessage));
        ...
        chatService.getGroupsByRoomId(chatMessage.getRoomId())
                .forEach(group ->
                        kafkaTemplate.send("/queue/user/" + group.getUser().getId(), chatMessage));
    }
...
//        template.convertAndSend("/topic/room/" + readMessageDto.getRoomId(), readMessageDto);
        ...
        kafkaTemplate.send("/topic/room/" + readMessageDto.getRoomId(), readMessageDto);
    }
...
//        template.convertAndSend("/topic/room/" + chatMessage.getRoomId(), readMessageDto);
        ...
        kafkaTemplate.send("/topic/room/" + chatMessage.getRoomId(), readMessageDto);
    }
}
